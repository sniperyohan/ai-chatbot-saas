...
function SubscriptionCard({ tenantPlan }: { tenantPlan: string }) {
  ...
  const dday = sub?.dday ?? sub?.days_until_billing
  const nextBilling = sub?.subscription_end_date || sub?.expires_at
...
export default function DashboardPage() {
  ...
  const scenarioLimit = getPlan(tenant?.plan).scenarioLimit
 ...
 && (
        <div style={{ ...S.card }}>
          ...
          <UsageBar label="시나리오" current={stats?.scenario_count || 0} limit={scenarioLimit} color="#F59E0B"/>
